...
class NycShows::CLI
  ...
  def command
    ...
    puts "Type 'off' for all off-broadway shows."
    ...
    def all_shows
      ...
      show_list = shows.each_with_index(1) do |show, index|
        puts "#{index}. #{show.name.titleize} - where: #{show.location} - length: #{show.duration} - genre: #{show.genre}"
      end
      show_list
    end
    ...
    def off_broadway
      ...
      off_b_list = off_b_shows.each_with_index(1) do |show, index|
        puts "#{index}. #{show.name.titleize}"
      end
      off_b_list
    end
